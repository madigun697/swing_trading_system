...
class PullbackStrategy:
    ...
    def generate(
        ...
        near_ma20 = (
            feature.ma_20 is not None and abs(close - feature.ma_20) / close <= 0.04
        )
        near_ma50 = (
            feature.ma_50 is not None and abs(close - feature.ma_50) / close <= 0.06
        )
        if not (0.02 <= pullback_pct <= 0.12 and (near_ma20 or near_ma50)):
            return None
        entry = close
        ma_stop = (
            (feature.ma_50 - atr) if feature.ma_50 is not None else entry - (2.0 * atr)
        )
        ...
        if risk_per_share <= 0 or position_size <= 0:
            return None
        risk_multiple_target = 2.5 if has_quality_or_rs_strength(candidate) else 2.0
        target = max(entry + (risk_multiple_target * risk_per_share), recent_high)
